...
# further bootstrap the train set
if args.bootstrap:
    kf_boot = KFold(n_splits=10,shuffle=True,random_state=args.bootstrap_seed)
    train_features=[train_features[i] for i in list(kf_boot.split(train_features))[0][0]]
    train_targets=[train_targets[i] for i in list(kf_boot.split(train_targets))[0][0]]
    train_masks=[train_masks[i] for i in list(kf_boot.split(train_masks))[0][0]]

print(len(train_features),len(train_targets),len(train_masks))
...
torch.save(model.state_dict(),trial_name+'_final.pt')
